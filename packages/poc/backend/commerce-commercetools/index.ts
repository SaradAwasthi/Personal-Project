...
import {MiraklProductPrice} from './apis/MiraklProductPriceApi'
...
export default {
  'dynamic-page-handler': async (
    ...
    if (ProductRouter.identifyFrom(request)) {
      return ProductRouter.loadFor(request, context.frontasticContext).then(async(product: Product) => {
        if (product) {
          //Mirakl Api fetching offers
          const variants = product.variants
          const priceApi = new MiraklProductPrice();

          for (const variant of variants) {
            if (!variant?.price?.fractionDigits) {
              console.log("variant", variant)

              const price = await priceApi.getProductDetails(variant.sku)
              console.log("price", price)
              if (price) {
                variant.offers = price[0]?.offer
              }
            }
    ...
    if (CategoryRouter.identifyFrom(request)) {
      return CategoryRouter.loadFor(request, context.frontasticContext).then(async (result: Result) => {
        if (result) {
          //mirakl product price for product list
           const priceApi = new MiraklProductPrice();

           for (const product of result.items) {
             const prodtransform: Product = product as Product
             console.log("itemArray", product)
             for (const variant of prodtransform.variants) {
               if (!variant?.price?.fractionDigits) {
                 console.log("variant", variant)
